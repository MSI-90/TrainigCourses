...
namespace WinTest
{
    public partial class FormMain : Form
    {
        ...
        private async void buttonEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                dataGridViewMain.DataSource = await Operation.GetCourses();
                MessageBox.Show($"Изменение для {id}: {course.Title} произведено успешно", "Информация",
        ...
        private async void buttonDel_Click(object sender, EventArgs e)
        {
            if (dataGridViewMain.RowCount == 0)
            {
                MessageBox.Show("Записи для удаления не найдены", "Ошибка",
            ...
            {
                dataGridViewMain.DataSource = await Operation.GetCourses();
                MessageBox.Show($"Удаление {id} произведено успешно", "Информация",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
